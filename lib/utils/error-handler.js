...
var exitCode = 0
var rollbacks = arduboy.rollbacks
...
process.on('exit', function (code) {
  log.disableProgress()
  if (!arduboy.config || !arduboy.config.loaded) return
    ...
    if (wroteLogFile) {
      ...
      log.error(
        ...
        [
          ...
          '    ' + path.resolve('arduboy-debug.log')
        ].join('\n')
      )
      wroteLogFile = false
    }
  ...
  var doExit = arduboy.config.get('_exit')
  if (doExit) {
    // actually exit.
...
function exit (code, noLog) {
  ...
  var doExit = arduboy.config ? arduboy.config.get('_exit') : true
  ...
  if (rollbacks.length) {
    ...
    rm('arduby-debug.log', reallyExit)
  }
...
function errorHandler (er) {
  ...
  if (!arduboy.config || !arduboy.config.loaded) {
  ...
  // log.error('arduboy -v', arduboy.version)
...
function writeLogFile (cb) {
  ...
  var fstr = writeStreamAtomic('arduboy-debug.log')
  var os = require('os')
  var out = ''
