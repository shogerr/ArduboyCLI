...
function generateFile(type, cb) {
  // TODO Remove this to an external file or source another repo/package 
  var inoTemplate = [
    '#include <Arduboy.h>',
    '',
    'Arduboy arduboy',
  ...
  switch (type) {
    case 'cpp':
      fileTemplate = cppTemplate
      break
    case 'sketch': 
      fileTemplate = inoTemplate
      break
    default:
  }
  ...
  return generateFilePrompt(dir, initFile, arduboy.config, fileTemplate, noProgressTillDone(generateCB))
}
...
function generateFilePrompt (dir, input, config, d, cb) {
