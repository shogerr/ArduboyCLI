...
function help (args, cb) {
  var argv = arduboy.config.get('argv').cooked
  ...
  // arduboy help foo bar baz: search topics
  /*
  if (args.length > 1 && args[0]) {
    return arduboy.commands['help-search'](args, argnum, cb)
  }
  */

  var section = arduboy.deref(args[0]) || args[0]

  // arduboy help <noargs>:  show basic usage
  if (!section) {
    var valid = argv[0] === 'help' ? 0 : 1
    return arduboyUsage(valid, cb)
  }

  // arduboy <cmd> -h: show command usage
  if (arduboy.config.get('usage') &&
      arduboy.commands[section] &&
      arduboy.commands[section].usage) {
    arduboy.config.set('loglevel', 'silent')
    log.level = 'silent'
    console.log(arduboy.commands[section].usage)
    return cb()
  }

  // arduboy apihelp <section>: Prefer section 3 over section 1
  ...
  // arduboy help <section>: Try to find the path
  ...
  // find either /section.n or /arduboy-section.n
  ...
  var f = '+(arduboy-' + section + '|' + section + ').[0-9]?(' + compextglob + ')'
  return glob(manroot + '/*/' + f, function (er, mans) {
    if (er) return cb(er)

    if (!mans.length) return arduboy.commands['help-search'](args, cb)
...
function viewMan (man, cb) {
  ...
  env.MANPATH = manpath
  var viewer = arduboy.config.get('viewer')
  ...
  switch (viewer) {
    ...
    case 'browser':
      opener(htmlMan(man), { command: arduboy.config.get('browser') }, cb)
      break
...
function arduboyUsage (valid, cb) {
  arduboy.config.set('loglevel', 'silent')
  log.level = 'silent'
  console.log([
    '\nUsage: arduboy <command>',
    '',
    'where <command> is one of:',
    arduboy.config.get('long') ? usages()
        : '    ' + wrap(commands),
    '',
    'arduboy <cmd> -h     quick help on <cmd>',
    'arduboy -l           display full usage info',
    'arduboy help <term>  search for help on <term>',
    'arduboy help arduboy involved overview',
    '',
    'Specify configs in the ini-formatted file:',
    '    ' + arduboy.config.get('userconfig'),
    'or on the command line via: arduboy <command> --key value',
    'Config info can be viewed via: arduboy help config',
    '',
    'arduboy@' + arduboy.version + ' ' + path.dirname(__dirname)
  ].join('\n'))
  cb(valid)
}
...
function usages () {
  ...
  var maxLen = 0
  return Object.keys(arduboy.commands).filter(function (c) {
    return c === arduboy.deref(c)
  }).reduce(function (set, c) {
    set.push([c, arduboy.commands[c].usage || ''])
